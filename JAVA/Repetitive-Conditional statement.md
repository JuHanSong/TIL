...
* if문과는 달리 조건식이 사용되지 않고, 값을 가지는 변수 또는 표현식이 판단조건으로 사용되며, 연산 결과의 데이터 타입은 int, String이 사용됩니다.
...
* for문은 제어조건을 한꺼번에 지정한다는 점이 다른 반복문과는 다릅니다. 따라서 정확한 반복 횟수를 알고 있을 때는 for문이 while문보다 유용합니다.
